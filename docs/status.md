...
| Sabiha | Phases 0-5 built and merged: FastAPI service, danger gate (100/100 recall, keyword-only, embedding path retired, response_override wired for abuse_violence), 3-band confidence pipeline, Gemini→Groq→static fallback, Supabase logging (scores field fixed), cache with hit-rate tracking, rate limiting, demo mode wired into band decision, tier enforcement, feedback endpoint + Supabase feedback_logs, output filter (incl. adversarial-injection test), constrained-generation + LLM-fallback tests, ADR 0003 (danger-gate matching) | Demo rehearsal script (scripts/demo_rehearsal.py) hits a plain POST /ask, which the live Gradio Space doesn't expose (404 on every query, script doesn't check for it) -- needs the same gradio_client fix keep_warm.py already has, then a real rerun before the 10-clean-runs item counts as done. |
