...
**Current phase:** Phase 2 — Make it a service. Phase 1 checklist fully closed 2026-09-01 (GO, fused Recall@5 = 0.971). Phase 2 started same day: `retrieval/pipeline.py`'s `search()` built. Live-verified 2026-09-02: warm loading/latency/memory effectively done (a real GPU-device bug in NLLB was found and fixed along the way — was 25000-60000ms/query, now ~1s). **Gold-set individual review in progress:** the 151 never-flagged legacy rows are now done (39 corrected, 23 dropped — see below); the 105 rows that only got the lighter category-boundary pass are the one remaining chunk. Once that's done, τ threshold tuning (blocked on gold-set quality, not code — correct/incorrect score distributions currently overlap too heavily) should be re-run.

## Who's on what

| Person | Currently | Blocked on |
|---|---|---|
| Sana | Levers 1, 3, 4 done and measured; reranking (item 8) built and measured; Sindhi+English fully embedded (4000/4000 points, verified live); gold eval set at 273 rows after individually reviewing the 25 new + 151 never-flagged legacy rows; Phase 1 go/no-go is a GO, checklist fully closed; Phase 2: pipeline.py latency/memory fixed and verified, τ threshold tuning still blocked pending the last 105 legacy rows | Gold-set review of the remaining 105 bulk-confirmed legacy rows — top priority, blocking Lever 5 threshold tuning |
