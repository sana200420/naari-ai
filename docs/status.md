...
**Current phase:** Phase 2 — Make it a service. Phase 1 checklist fully closed 2026-09-01 (GO, fused Recall@5 = 0.971). Phase 2 started same day: `retrieval/pipeline.py`'s `search()` built. Item 1 (warm loading) and Item 2 (conditional English leg) are both done. **Item 3 (τ tuning) is settled as un-reachable at the 0.95 target, not still-mysterious**: properly cross-validated, no confidence signal or combination gets past ~84-90% precision, so the plan shifted from "find the right threshold" to (1) a three-band verbatim/confirm/decline design and (2) fixing the reranker directly — `retrieval/pipeline.py` now has a tie-break guard (`_prefer_fusion_top1_if_close`) for the specific demotion pattern found in the audit (fusion's own top-1 pick, already correct, getting overridden by the reranker in ~10% of queries), shipped 2026-09-07 with its margin still uncalibrated (defaults to a no-op). See `eval/results.md`'s 2026-09-07 section and the Lever 5 line below.

## Who's on what

| Person | Currently | Blocked on |
|---|---|---|
| Sana | Levers 1, 3, 4 done and measured; reranking (item 8) built and measured; Sindhi+English fully embedded (4000/4000 points, verified live); gold eval set at 248 rows, every row individually reviewed, now also carrying `acceptable_answer_ids` for verified near-duplicate KB rows; Phase 1 go/no-go is a GO; Phase 2 Items 1 and 2 done, Item 3 (τ tuning) resolved as "0.95 unreachable, shifted to a three-band design + a shipped-but-uncalibrated reranker fix" | Sweep `RERANK_OVERRIDE_MARGIN` against Recall@1 (data already exported, no GPU run needed for the sweep itself), pick a value, re-measure |
...
## Phase 2, Item 3 (τ tuning) — resolved as "unreachable at 0.95," reranker fix shipped 2026-09-07
