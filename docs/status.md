...
| Sabiha | Phases 0-5 built and merged: FastAPI service, danger gate (100/100 recall, keyword-only, embedding path retired), 3-band confidence pipeline, Gemini→Groq→static fallback, Supabase logging, cache/rate-limit/demo-mode, tier enforcement wired into pipeline Stage 02b (PR #22), audio router registered in main.py (PR #22), output filter tests (dose/false-reassurance/safe-passthrough), feedback endpoint + Supabase feedback_logs table (PR pending review) | Refusal correctness at 0.860 vs 0.95 target (scope-classifier gap on abortion/DV/contraceptives — Sana's definition_of_done.py scorecard); PR #23 partially addressed this, Sana's measured 86-keyword list not yet fully applied |
