...
- [ ] **Embed 2000 Sindhi + 2000 English rows; create the Qdrant collection** — *Done when* it holds 4000 points with named dense/sparse vectors and a `lang` payload. — **3999/4000 as of 2026-09-01** (verified live against the collection): all 2000 Sindhi rows + 1999/2000 English rows indexed. `id=2000` (the backfilled Sindhi row replacing the original blank row 28) has no English translation yet — it's a genuinely new question, not present in any contributor's original file, so it needs a real translation written, not a data-merge fix. Blocking full completion of this item.
...
- [x] **Add the sparse leg and RRF fusion; produce the ablation table** — *Done when* dense-only, sparse-only and fused Recall@5 appear in one committed table. `[Lever 3]` — `retrieval/search.py` (`HybridRetriever`, `reciprocal_rank_fusion`), table in `eval/results.md`. **Caveat, found 2026-09-01:** the committed provisional table was measured before a real bug fix — `dense_search`/`sparse_search` had no `lang` filter, so once the English KB shared the collection, "Sindhi-only" search was silently returning English points too, double-counting some answer_ids in RRF. Fixed (`lang="sd"` now the default). The queued final pass in `retrieval/scripts/link_and_baseline_gold_queries.ipynb` (human-reviewed 275-query gold set, needs a live Colab run) will produce the first trustworthy numbers.
- [ ] **Add the English dual path with local NLLB translation** — *Done when* you can state what fraction of Sindhi-only failures English rescues. `[Lever 4]` — `retrieval/translate.py` (NLLB-200-distilled-600M) + `HybridRetriever.cross_lingual_search()` (SD dense + SD sparse + translated-query EN dense, fused by `answer_id`) built and unit-tested against fakes. Still needs a live Colab run against the gold set to actually state the rescue fraction — code correctness ≠ the measurement this item asks for.
...
- [ ] **Phase 1 go/no-go on the embedding model** — *Done when* Recall@5 ≥ 0.85, or a written decision to fine-tune on Colab with a named date. `[Risk 1]` — blocked on the final (human-reviewed) recall number and the two items above.
