...
- [ ] **Convert both models to ONNX int8** — *Done when* p95 retrieval latency is under 1.5s on the free Space and recall is within 1 point of float32. `[Risk 2]` — **Notebook built 2026-09-09, not yet run.** `retrieval/scripts/convert_models_to_onnx_int8.ipynb` exports and quantizes the embedder's dense output and the reranker, with two hard sanity-check gates (reranker relevant/irrelevant-pair agreement, dense-embedding cosine similarity >0.95) that stop the notebook before the expensive full benchmark if the ONNX conversion doesn't match the original model. **Scope decision:** sparse search stays on the original float32 model — its extra learned head isn't exportable via standard tooling (`optimum`), and converting it without a way to test locally first risked silently corrupting a component that's currently pulling real weight (0.542 Recall@1 alone, better than dense alone). Latency is benchmarked on CPU specifically (not this notebook's GPU), since the actual deployment target ("the free Space") is CPU-only. **This item stays unchecked until the notebook is actually run and both the latency and recall gates report PASS** — no numbers exist yet to justify checking it off.
