...
- [ ] **Package retrieval as an importable module, models loaded once at startup** — *Done when* a second query returns under 1s and memory is flat across 100 requests. `retrieval/pipeline.py`'s `search()` built (2026-09-01): matches `docs/contracts/retrieval.json` exactly, warm-loading falls out of the existing lazy singletons for free, 9 tests against fakes pass. **Not yet checked off** — the "done when" bar is a live latency/memory measurement, not code existing; needs a notebook pass against real Qdrant + real models.
- [ ] **Make the English leg conditional on the Sindhi top score** — *Done when* median latency drops measurably and Recall@5 does not. `[Lever 4] [Risk 2]` — implemented in `retrieval/pipeline.py`'s `search()`: gates on the Sindhi *reranked* top score vs `tau_high` (env var, 0.75 fallback), only then translates + runs the English leg + reranks the combined pool. Tested against fakes in both directions. **Not yet checked off** — needs the live latency/recall comparison against the gold set.
- [ ] **Tune τ_high and τ_low from score distributions and the negative set** — *Done when* the precision/coverage curve is committed as a figure and both thresholds have a stated justification. `[Lever 5]` — **unblocked as of 2026-09-01**: `eval/negative_set_100.csv` exists and checks out (100 rows, 100 distinct subcategories, all 8 categories, plausible no-KB-answer questions, 0 dupes/blanks). Not yet run.
