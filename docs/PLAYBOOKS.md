...
- [ ] **Add cross-encoder reranking over the fused top 20** — *Done when* the Recall@1 improvement from reranking is measured and committed. `retrieval/rerank.py` built and tested (2026-09-01); first live measurement came back Recall@1 = 0.385, *below the weakest single leg* — a stronger sign of an ordering/pairing bug than a real capability gap, so **not accepted as a final result yet**. Diagnostic cells added to the notebook (sanity-check `compute_score` in isolation + print actual before/after disagreements), not yet run. Not blocking Phase 1's GO (fused alone already clears the exit gate).
