...
- [ ] **Tune τ_high and τ_low from score distributions and the negative set** — *Done when* the precision/coverage curve is committed as a figure and both thresholds have a stated justification. `[Lever 5]` — **Gold-set review complete** (2026-09-02, 248 rows, all individually verified). Re-run 2026-09-05 with real diagnostics (not just more guessing): `tau_low = 0.2034` converges fine; **`tau_high` still doesn't converge, but now we know precisely why.** Recall@1 (0.339) vs. Recall@20 pre-rerank (0.726) is a 39-point gap — the correct answer is usually *in* the shortlist, so this is a ranking/reranking problem, not retrieval/embedding. Sharper: of 58 incorrect-but-score≥0.9 queries audited, **25 had the correct answer sitting at rank 1 of the pre-rerank fused shortlist** — the reranker is actively demoting an already-correct answer for ~10% of all queries, not just occasionally confusing categories (`gold_7` from the Item 8 audit is one of these 25, confirming it recurs at scale, not a one-off). Corrected finding: the notebook's first pass had an English-vs-Sindhi category-label bug reporting a false 58/58 category-mismatch rate; true split is 27/58 cross-category, 31/58 same-category-wrong-row — fixed in the notebook. Full writeup: `eval/results.md`'s 2026-09-05 section. **Next step:** capture Diagnostic 4's margin/`fusion_agrees_with_final` precision-coverage numbers (printed but not yet saved to a file) — fusion-agreement is the strongest candidate replacement signal given the rank-1-demotion finding — then decide the fix: reranker retuning, a fusion-agreement gate, or skipping reranking when fusion's own top-1 score is already high.
