...
- [ ] **Tune τ_high and τ_low from score distributions and the negative set** — *Done when* the precision/coverage curve is committed as a figure and both thresholds have a stated justification. `[Lever 5]` — **Gold-set review complete** (2026-09-02, 248 rows). `tau_low = 0.2034` converges fine. **`tau_high` does not converge, and as of 2026-09-07 this is now a settled finding, not an open question:** properly cross-validated (10-fold, pooled predictions — the single-split numbers from the first pass were optimistic noise, one query swings precision ~1.3pts on a 75-row test fold), no signal or combination (raw score, margin, fusion-agreement, category-match, or a fitted logistic regression over all four) reaches 0.95 precision at any coverage down to a few dozen queries — the best is ~83.6% precision at 24.6% coverage (logreg), or ~90.6% at 12.9% coverage (the most extreme hand-built rule). **This changes the plan**, not just the number: (1) ship a three-band design (verbatim / "did you mean" confirmation / decline) instead of chasing 0.95 on a binary threshold — 85-90% precision is a reasonable bar for a confirmation step; (2) fix the underlying cause instead of only gating around it. Root cause, confirmed 2026-09-05: of 58 incorrect-but-score≥0.9 queries, 25 had the correct answer at rank 1 of the *pre-rerank* fused shortlist — the reranker actively demotes an already-correct answer for ~10% of all queries (`gold_7` from the Item 8 audit is one of these). **Shipped 2026-09-07:** `retrieval/pipeline.py`'s `_prefer_fusion_top1_if_close()` — a tie-break that keeps fusion's #1 pick when the reranker's alternative beats it by less than `RERANK_OVERRIDE_MARGIN` (default `0.0`, a no-op pending calibration; tested in `retrieval/tests/test_pipeline.py`). `eval/gold_scored_full.csv` now also exports the pre-guard reranker state needed to sweep this margin offline, no extra GPU run per candidate value. **Next step:** run that sweep against Recall@1, pick the margin, re-measure. Full writeup: `eval/results.md`'s 2026-09-07 section.
