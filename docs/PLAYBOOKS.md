...
- [ ] **Tune τ_high and τ_low from score distributions and the negative set** — *Done when* the precision/coverage curve is committed as a figure and both thresholds have a stated justification. `[Lever 5]` — Run 2026-09-02: `tau_low = 0.2034` (hits the ≥90% negative-set target), but no `tau_high` threshold reached 0.95 precision anywhere — `eval/tau_score_distributions.png` shows correct/incorrect gold-query scores overlapping heavily even near 1.0. Direct consequence of the Item 8 audit. **Gold-set review now fully complete** (2026-09-02): every row in `eval/gold_eval_280_linked.csv` has been individually verified against the KB — the 25 new rows (`eval/new25_review_final.csv`: 11 OK, 10 corrected, 4 dropped), the 151 never-flagged legacy rows (`eval/never_flagged_151_review.csv`: 39 corrected, 23 dropped), and the 105 bulk-confirmed legacy rows (`eval/bulk_confirmed_105_review.csv`: 54 corrected, 25 dropped). Set is now 248 rows, no more coarse category-boundary-only acceptances anywhere. **Ready to re-run:** the τ_high/τ_low tuning notebook against this cleaned set is the next step — this item stays unchecked until that re-run produces a real threshold.
